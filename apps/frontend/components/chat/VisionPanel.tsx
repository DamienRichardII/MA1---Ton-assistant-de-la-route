...
export function VisionPanel() {
  ...
  // [Sprint Étape 2] Sanitisation XSS — plus de dangerouslySetInnerHTML.
  // Échappe tout HTML reçu de l'IA puis rend **gras** et sauts de ligne en composants React.
          ...
          <div className="glass rounded-3xl p-5">
            ...
            : result ? <>
              ...
              <div className="text-sm leading-relaxed text-white/70">
                {escapeHtml(result).split(/\n/).map((line, li, all) => {
                  // Rend uniquement **gras** sur du texte déjà échappé. Aucun autre tag injecté.
                  const parts = line.split(/(\*\*[^*]+\*\*)/g).map((seg, si) => {
                    const m = seg.match(/^\*\*([^*]+)\*\*$/);
                    return m ? <strong key={si}>{m[1]}</strong> : <span key={si}>{seg}</span>;
                  });
                  return <span key={li}>{parts}{li < all.length - 1 ? <br /> : null}</span>;
                })}
              </div>
